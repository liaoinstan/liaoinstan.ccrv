...
public class CalendarGridView extends ViewGroup {
   ...
   */
  private static final float FLOAT_FUDGE = 0f;
  private float PAINT_WIDTH = 6f;	//add by lz ∑÷∏Ùœﬂ¥÷œ∏
  ...
  public CalendarGridView(Context context, AttributeSet attrs) {
    ...
    if (!isInEditMode()) {
    	...
    	dividerPaint.setStrokeWidth(PAINT_WIDTH);
	}
  }
  ...
  @Override protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
    ...
    final int bottom = (int) (child.getBottom() -  Math.ceil(PAINT_WIDTH/2));
//    canvas.drawLine(child.getLeft(), bottom, child.getRight() - 2, bottom, dividerPaint);
    canvas.drawLine(child.getLeft(), bottom, child.getRight(), bottom, dividerPaint);//modify by lz
    return retVal;
  }
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    //modify by lz
    final int measuredWidth = widthMeasureSize + 6; // Fudge factor to make the borders show up.
